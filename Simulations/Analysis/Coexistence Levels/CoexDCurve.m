...
title('Path of equilibrium point for 0 < d < 10')

%%% equilib population values as function of d
figure()
hold on; box on;
plot(D,Epts(:,1),'Linewidth',2)
plot(D,Epts(:,2),'Linewidth',2)
xlabel('b')
ylabel('Scaled populations at equilibrium')
legend('C','F')
title('Equilbrium values as function of death rate (d))')
...
col = [D(1:end-1), NaN]; % Must end in NaN to filling a solid
