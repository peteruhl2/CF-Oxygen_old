...
%%% array for results =====================================================
% Eta = linspace(3e-4,1e-1);
Eta = 10.^linspace(-4,-2);
Epts = zeros(length(Eta),2);
V = zeros(length(Eta), 2);
...
title('Path of equilibrium point for 10^-4 < eta < 10^-2')

%%% equilib population values as function of q
figure()
hold on; box on;
plot(Eta,Epts(:,1),'Linewidth',2)
plot(Eta,Epts(:,2),'Linewidth',2)
xlabel('q')
ylabel('Scaled populations at equilibrium')
legend('C','F')
title('Equilbrium values as function of oxygen consumption (eta)')
...
col = [Eta(1:end-1), NaN]; % Must end in NaN to filling a solid
