...
%%% array for results =====================================================
LambdaFrac = linspace(0.1,4);
Epts = zeros(length(LambdaFrac),2);
V = zeros(length(LambdaFrac), 2);
...
title('Path of equilibrium point for 9.6 x 10^6 < lambda < 3.84 x 10^8')

%%% equilib population values as function of q
figure()
hold on; box on;
plot(LambdaFrac,Epts(:,1),'Linewidth',2)
plot(LambdaFrac,Epts(:,2),'Linewidth',2)
xlabel('q')
ylabel('Scaled populations at equilibrium')
legend('C','F')
title('Equilbrium values as function of oxygen inflow (lambda)')
...
col = [LambdaFrac(1:end-1), NaN]; % Must end in NaN to filling a solid
