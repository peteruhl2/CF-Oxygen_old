...
%%% array for results =====================================================
MuFrac = linspace(0,10,100);
Epts = zeros(length(MuFrac),2);
V = zeros(length(MuFrac), 2);
...
hold on
for i = 1:length(MuFrac)
    %%% new value of q
    mu = 200*23*60*24*MuFrac(i);
...
title('Path of equilibrium point for 0 < mu < 6.6 x 10^7')

%%% equilib population values as function of d
figure()
hold on; box on;
plot(MuFrac,Epts(:,1),'Linewidth',2)
plot(MuFrac,Epts(:,2),'Linewidth',2)
xlabel('b')
ylabel('Scaled populations at equilibrium')
legend('C','F')
title('Equilbrium values as function of oxygen decay rate (mu))')
...
col = [MuFrac(1:end-1), NaN]; % Must end in NaN to filling a solid
