...
title('Path of equilibrium point for 0 < r < 100')

%%% equilib population values as function of d
figure()
hold on; box on;
plot(R,Epts(:,1),'Linewidth',2)
plot(R,Epts(:,2),'Linewidth',2)
xlabel('b')
ylabel('Scaled populations at equilibrium')
legend('C','F')
title('Equilbrium values as function of attack growth rate (r))')
...
col = [R(1:end-1), NaN]; % Must end in NaN to filling a solid
