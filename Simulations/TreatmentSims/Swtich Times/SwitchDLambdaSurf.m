...
%%% this one a surface for lambda and d
...
LambdaFrac = linspace(0.75,1.15,res);
D = linspace(0,7,res);
results = zeros(length(LambdaFrac),length(D));
...
[X,Y] = meshgrid(LambdaFrac,D);

hold on; box on;
% surf(X,Y,Fvals')
contourf(X,Y,results')
xlabel('Oxygen inflow rate \lambda')
ylabel('Death rate d')
h2 = colorbar;
