%%% surf of lambda and epsilon vs exacerbation time
%%% 5/12/2020
...
res = 50;
Ep = linspace(0.5,4.302,res);
LambdaFrac = linspace(0.6,1.2,res);
results = zeros(length(Ep),length(LambdaFrac));

for i = 1:length(Ep)
    for j = 1:length(LambdaFrac)
        [i j]
        ...
        k = 10^10;
        eta = 3.16e-4; % increased a bit for simulations
        q = 3.2747e-5;

        frac = 0.8659;

        % lambda = mu*x0;
        lambda = 9.6901e+07*LambdaFrac(j);
...
figure()
[X,Y] = meshgrid(Ep,LambdaFrac);
contourf(X,Y,results');
colorbar
xlabel('Clindamycin strength')
ylabel('Oxygen inflow (% of normal)')
